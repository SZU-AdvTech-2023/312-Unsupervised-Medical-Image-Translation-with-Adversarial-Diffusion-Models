...
def sample_from_model(coefficients, generator, n_time, x_init, T, opt):
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            # x_0 = gen(x + source, time, noise)
            x_0 = generator(torch.cat((x,source),axis=1), t_time, latent_z)
            x_new = sample_posterior(coefficients, x_0[:,0:3,:], x, t)
            x = x_new.detach()
...
def train_syndiff(rank, gpu, args):
    ...
    #                                t_emb_dim = args.t_emb_dim,
    #                                act=nn.LeakyReLU(0.2)).to(device)
    disc_diffusive_1 = Discriminator_large(nc = 6, ngf = args.ngf, 
                                   t_emb_dim = args.t_emb_dim,
                                   act=nn.LeakyReLU(0.2)).to(device)
    disc_diffusive_2 = Discriminator_large(nc = 6, ngf = args.ngf, 
                                   t_emb_dim = args.t_emb_dim,
                                   act=nn.LeakyReLU(0.2)).to(device)
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            ...
            x1_pos_sample = sample_posterior(pos_coeff, x1_0_predict_diff[:,0:3,:], x1_tp1, t1)
            x2_pos_sample = sample_posterior(pos_coeff, x2_0_predict_diff[:,0:3,:], x2_tp1, t2)
            ...
            errG1_L1 = F.l1_loss(x1_0_predict_diff[:,0:3,:],real_data1)
            errG2_L1 = F.l1_loss(x2_0_predict_diff[:,0:3,:],real_data2)
        ...
        if rank == 0:
            ...
            pred1 = torch.cat((real_data2, pred1, gen_non_diffusive_1to2(pred1), fake_sample2_tilda[:,0:3,:]),axis=-1)
            ...
            pred2 = torch.cat((real_data1, pred2, gen_non_diffusive_2to1(pred2), fake_sample1_tilda[:,0:3,:]),axis=-1)
